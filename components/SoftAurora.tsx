...
float auroraGlow(float t, vec2 shift) {
  ...
  // Reduced octaves for better performance (2 instead of 3)
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    // Check if drawing buffer exists
    const dpr = Math.min(window.devicePixelRatio, 2); // Limit DPR for performance
    const renderer = new Renderer({ 
      alpha: true, 
      premultipliedAlpha: false,
      powerPreference: 'low-power', // Favor battery/efficiency
      antialias: false // Disable antialias for speed as aurora is blurry anyway
    });
    ...
    function resize() {
      ...
      // Ensure we don't over-scale on high DPI
